...
async def back_menu(callback: CallbackQuery, session: AsyncSession) -> None:
    ...
    # Пытаемся удалить старое сообщение с картинкой карточки
    try:
        await callback.message.delete()
    except TelegramAPIError:
        pass

    # Отправляем главное меню новым чистым сообщением
...
async def show_catalog(callback: CallbackQuery, session: AsyncSession) -> None:
    ...
    try:
        ...
        # Если это было сообщение с картинкой, удаляем его и шлем текстом
        try:
            await callback.message.delete()
        except TelegramAPIError:
            pass
...
async def show_matched(callback: CallbackQuery, session: AsyncSession) -> None:
    ...
    try:
        await callback.message.edit_text(text, reply_markup=offers_list_keyboard(offers, category))
    except TelegramAPIError:
        try:
            await callback.message.delete()
        except TelegramAPIError:
            pass
        await callback.message.answer(text, reply_markup=offers_list_keyboard(offers, category))
...
async def show_category(callback: CallbackQuery, session: AsyncSession) -> None:
    ...
    await callback.answer()
    if not offers:
        text_empty = "В этой категории сейчас нет открытых вакансий."
        try:
            await callback.message.edit_text(text_empty, reply_markup=categories_keyboard())
        except TelegramAPIError:
            try:
                await callback.message.delete()
            except TelegramAPIError:
                pass
            await callback.message.answer(text_empty, reply_markup=categories_keyboard())
        return

    text_intro = format_category_intro(category)
    try:
        await callback.message.edit_text(text_intro, reply_markup=offers_list_keyboard(offers, category))
    except TelegramAPIError:
        try:
            await callback.message.delete()
        except TelegramAPIError:
            pass
        await callback.message.answer(text_intro, reply_markup=offers_list_keyboard(offers, category))
